...
import com.algaworks.algafood.domain.exception.EntidadeNaoVinculadaException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		ApiError error = createApiErrorBuilder(status, errorType, detail).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL)
				.build();
	...
	@ExceptionHandler(EntidadeNaoVinculadaException.class)
	public ResponseEntity<?> handleEntidadeNaoVinculada(EntidadeNaoVinculadaException ex, WebRequest request) {
	...
	public ResponseEntity<?> handleAccessDenied(AccessDeniedException ex, WebRequest request) {
		...
		String detail = ex.getMessage();
		String userMessage = "Você não possui permissão para executar essa operação.";

		ApiError error = createApiErrorBuilder(status, errorType, detail).userMessage(userMessage).build();
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		if (body == null) {
			body = ApiError.builder().status(status.value()).title(status.getReasonPhrase())
					.userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).timestamp(OffsetDateTime.now()).build();
		} else if (body instanceof String) {
			body = ApiError.builder().status(status.value()).title((String) body)
					.userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).timestamp(OffsetDateTime.now()).build();
	...
	private ApiError.ApiErrorBuilder createApiErrorBuilder(HttpStatus status, ApiErrorType errorType, String detail) {
		return ApiError.builder().status(status.value()).type(errorType.getUri()).title(errorType.getTitle())
